  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (status.isLoggedIn) {
        ...
        toast.custom(
          (t) => (
            <div className={`${t.visible ? "animate-enter" : "animate-leave"} max-w-xl w-full bg-white shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5`}>
              <div className="flex-1 w-0 p-4">
                <div className="flex items-start">
                  <div className="ml-3 flex-1">
                    <p className="text-xl font-bold text-gray-900">
                      Welcome to the community <b>{status?.userData?.username}</b> 🤗
                    </p>
                    <p className="mt-1 text-lg text-gray-500">Find your friends and start a room. Have a great time!</p>
                  </div>
                </div>
              </div>
      ...
      <form className="flex flex-col w-[400px] max-w-7xl mx-auto items-center justify-center px-5 py-10 gap-y-6 text-white bg-secondary rounded-xl" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="w-full relative group">
          <input
            type="email"
            className={`w-full rounded-xl h-18 px-6 py-6 text-base 
            ${errors.email ? "ring-1 ring-hardRed" : ""} 
            outline-none focus:ring-1 focus:ring-accent1 bg-grey3`}
            name="email"
