...
const Description = ({ isRoom, handleProblemChange, showParticipant, setShowParticipant }) => {
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col grow px-3 pt-6">
          ...
          <div className="flex flex-row items-center gap-x-5 mb-6">
            ...
            >
              {activeProblem?.difficulty?.charAt(0).toUpperCase() + activeProblem?.difficulty?.slice(1)}
            </div>
          ...
          <div className="mb-6 text-lg">{activeProblem?.statement && he.decode(activeProblem?.statement)}</div>
          ...
          <div className="flex flex-row flex-wrap items-center gap-x-5 mt-12 mb-12">
            <div className="flex flex-row items-center gap-x-3">
              ...
              <p className="text-xl font-bold">{formatStats(activeProblem?.stats?.accepted) || 0}</p>
            </div>
            <div className="flex flex-row items-center gap-x-3">
              <p className="text-grey1 text-base">Submissions</p>
              <p className="text-xl font-bold">{formatStats(activeProblem?.stats?.submissions) || 0}</p>
            </div>
            <div className="flex flex-row items-center gap-x-3">
              <p className="text-grey1 text-base">Acceptance</p>
              <p className="text-xl font-bold">{activeProblem?.stats?.acceptance || 0}%</p>
            </div>
          </div>
        </div>
      ) : (
